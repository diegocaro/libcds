...
namespace cds_static
{
    ...
    class wt_node_leaf: public wt_node
    {
        public:
            ...
            virtual void range_report(uint start, uint end, uint lowvoc, uint uppvoc, uint vocmin, uint vocmax, uint l, wt_coder *c, Mapper * am,vector<uint> &res) const;
            
            virtual uint next_value_pos_bin(uint number, uint start, uint end, uint vocmin, uint vocmax, uint l, wt_coder *c, uint *success) const;
	    
	    virtual void next_value_pos_huff(uint number, uint start, uint end, uint *min, uint *min_pos,uint *success) const;
